...
A **collection** data type is a grouping of data items
(possibly only zero or one item) that have some shared significance
...
of identical type in which each element can be identified by an array index. A C++
**array** is always stored in contiguous memory. C++ arrays can be allocated in two different ways:
...
In modern C++, the statically allocated array is typically used in situations
when speed is essential or where hardware constraints exist.

As a Python programmer, you might see the array as the ancestor
of the Python list, and you might remember that Python lists are actually implemented using
...
The fact that array elements are stored contiguously in memory locations makes look-up
via index very, very fast.
...
It is also possible to initialize into statically allocated arrays at compile time,
...
end of an array, but it will also let you change the values
